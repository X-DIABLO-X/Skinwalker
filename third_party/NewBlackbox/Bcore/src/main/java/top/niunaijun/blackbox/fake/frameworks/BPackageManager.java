...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        if (shouldUseFallbackMode()) {
            Log.w(TAG, "Virtual PackageManager unavailable; refusing host launch fallback for " + packageName);
            return null;
        }
    ...
    public boolean isInstalled(String packageName, int userId) {
        ...
        if (shouldUseFallbackMode()) {
            Log.w(TAG, "Virtual PackageManager unavailable; refusing host isInstalled fallback for " + packageName);
            return false;
        }
    ...
    private boolean isInstalledFallback(String packageName) {
        Log.w(TAG, "Host PackageManager fallback disabled for virtual install checks: " + packageName);
        return false;
    }
