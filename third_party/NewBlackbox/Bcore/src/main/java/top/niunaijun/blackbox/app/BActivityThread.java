...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        mProviders.clear();
        mProviders.addAll(sortProvidersForInitialization(Arrays.asList(packageInfo.providers)));
        ...
        if (Build.VERSION.SDK_INT >= 35) {
            Slog.w(TAG, "Skipping NativeCore.init on Android 15+ because getArtField is unavailable.");
        } else {
    ...
    private void installProviders(Context context, String processName, List<ProviderInfo> provider) {
        long origId = Binder.clearCallingIdentity();
        try {
            for (ProviderInfo providerInfo : sortProvidersForInitialization(provider)) {
                try {
                    if (processName.equals(providerInfo.processName) ||
                            providerInfo.processName.equals(context.getPackageName()) || providerInfo.multiprocess) {
                        Slog.d(TAG, "Installing provider " + providerInfo.name
                                + " authority=" + providerInfo.authority
                                + " process=" + providerInfo.processName
                                + " initOrder=" + providerInfo.initOrder);
                        installProvider(BlackBoxCore.mainThread(), context, providerInfo, null);
                    }
                } catch (Throwable e) {
                    Slog.e(TAG, "Provider install failed: " + providerInfo.name
                            + " authority=" + providerInfo.authority, e);
                }
            }
        } finally {
            Binder.restoreCallingIdentity(origId);
    ...
    private List<ProviderInfo> sortProvidersForInitialization(List<ProviderInfo> providers) {
        ArrayList<ProviderInfo> sortedProviders = new ArrayList<>(providers);
        Collections.sort(sortedProviders, (left, right) -> {
            int leftPriority = providerBootstrapPriority(left);
            int rightPriority = providerBootstrapPriority(right);
            if (leftPriority != rightPriority) {
                return Integer.compare(rightPriority, leftPriority);
            }
            return Integer.compare(right.initOrder, left.initOrder);
        });
        return sortedProviders;
    }

    private int providerBootstrapPriority(ProviderInfo providerInfo) {
        if (providerInfo == null || providerInfo.name == null) {
            return 0;
        }
        String name = providerInfo.name;
        if ("com.google.firebase.provider.FirebaseInitProvider".equals(name)
                || name.endsWith(".FirebaseInitProvider")) {
            return 1000;
        }
        if ("androidx.startup.InitializationProvider".equals(name)
                || name.endsWith(".InitializationProvider")) {
            return 900;
        }
        return 0;
    }
